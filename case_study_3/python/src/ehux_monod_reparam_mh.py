...
def build_pymc_model(times, total_obs, dead_obs):
    ...
    # Known from posterior ridge fit
    a = 3.671987e-13
    b = -2.389792e-12
    sigma_ridge = 1.170432e-11

    with pm.Model() as model:
        mu_max = pm.Uniform("mu_max", 0.4, 0.7)
        Ks = pm.Uniform("Ks", 0.05, 0.5)
        #Ks = pm.LogNormal("Ks",np.log(50), 1.0)
        ...
        # Reparameterized Qn (no longer sampled)
        Qn = pm.Deterministic(
            "Qn",
            a * N0 + b
        )
...
if __name__ == "__main__":
    ...
    file_path = '../res/ehux_monod_reparam_mh4.nc'
    # Build and run model
    ...
    if plot_posterior_pairs_flag:
        ...
        hspace=0.3,
        wspace=0.3,
        save_path='../figures/ehux_monod_posterior_reparam_mh4.svg'
        )
    ...
    if plot_convergence_flag:
        plot_convergence(
            ...
            var_order=['mu_max','delta','N0','P0','D0','sigma_live','sigma_dead'],
            figsize=(10, 10),
            save_path="../figures/ehux_monod_convergence_reparam_mh4.svg"
            )
...
'''
        if "Qn" not in posterior:
            a = 3.671987e-13
            b = -2.389792e-12
            N0_vals = posterior["N0"].values
            Qn_vals = (N0_vals - b) / a  # Rearranging the equation Qn = (N0 - b) / a
            posterior["Qn"] = xr.DataArray(
            Qn_vals,
            dims=posterior["N0"].dims,
            coords=posterior["N0"].coords
            )
