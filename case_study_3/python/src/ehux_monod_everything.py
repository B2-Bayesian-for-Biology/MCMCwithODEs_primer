...
class SolveIVPWrapper(Op):
    ...
    def perform(self, node, inputs, outputs):
        ...
        if not sol.success:
            outputs[0][0] = np.full((len(self.times), 3), np.nan)
            return
...
def build_pymc_model(times, total_obs, dead_obs):
    ...
    with pm.Model() as model:
        ...
        #mu_max = pm.ConstantData("mu_max", 0.55)
        
        Ks = pm.Uniform("Ks", 0.05, 0.4)
        #Ks = pm.ConstantData("Ks", 0.1)

        Qn = pm.Uniform("Qn", 2e-10, 30e-10)
        #Qn = pm.LogUniform("Qn", lower=2e-10, upper=30e-10)

        delta = pm.Normal("delta", mu = 0.034, sigma = 0.003)
        #delta = pm.ConstantData("delta", 0.035)
        #delta = pm.Uniform("delta", 0.01, 0.09)

        #N0 = pm.Deterministic("N0", 1000 + ((500 / 1.8e-10) * (Qn - 3.2e-10)))
        #N0 = pm.Deterministic("N0", 500 + ((1000 / 3.6e-10) * (Qn - 1.9e-10)))
        N0 = pm.TruncatedNormal("N0", mu=880, sigma=100,lower=200, upper=2000)

        P0 = pm.Normal("P0", mu=211017, sigma=14380)  # Based on data, around
        #P0 = pm.ConstantData("P0",1.76e5 )

        D0 = pm.Normal("D0", mu=31104, sigma=5761)  # Based on data, aroun        
        #D0 = pm.ConstantData("D0", 32000 )

        sigma_live = pm.HalfNormal("sigma_live", 1)
        sigma_dead = pm.HalfNormal("sigma_dead", 1)
        #sigma_live = pm.ConstantData("sigma_live", 0.1)
        #sigma_dead = pm.ConstantData("sigma_dead", 0.2)
        ...
        eps = 1* 1e-12
        pm.Normal("Y_total", mu=pt.log(total+eps), sigma=sigma_live, observed=np.log(total_obs))
        pm.Normal("Y_dead", mu=pt.log(dead+eps), sigma=sigma_dead, observed=np.log(dead_obs))

    return model


def run_inference(model, draws=20000, tune=20000, chains=3, cores=3, threshold_for_slice=0,target_accept=0.9):
    with model:
...
if __name__ == "__main__":
    ...
    if plot_trace_flag:
        ...
        save_path='../figures/ehux_monod_trace_everything_mh.svg'
        )
    ...
    if plot_posterior_pairs_flag:
        ...
        hspace=0.3,
        wspace=0.3,
        save_path='../figures/ehux_monod_posterior_everything_mh.svg'
        )
    ...
    if plot_convergence_flag:
        plot_convergence(
            ...
            var_names_map = {"mu_max":r'Maximum growth rate, $μ_{max}$ (/day)', "Ks":r"Half saturation constant mmol$/m^3$ ", "P0":r'Init. living cells, $P_0$ (cell/ml)',
                         "D0":r'Init. dead cells, $D_0$ (cell/ml)',"N0":r'Init. nutrient concentration, $N_0$ (mmol$/m^3$)',"sigma_live":"Standard Deviation, $\sigma_{LL,1}$","sigma_dead":"Standard Deviation, $\sigma_{LL,2}$",
                         "delta":"Death Rate, δ (/day)",'Qn': r'Nutrient Quota $Q_N$ (mmol N/cell)'},
            var_order=['mu_max','delta','Ks','Qn','N0','P0','D0','sigma_live','sigma_dead'],
            figsize=(10, 10),
            save_path="../figures/ehux_monod_convergence_everything_mh.svg"
            )
    ...
    if plot_dynamics_flag:
        ...
        if "N0" not in posterior:
            ...
            import xarray as xr
            posterior["N0"] = xr.DataArray(
                N0_vals,
                dims=posterior["Qn"].dims,
                coords=posterior["Qn"].coords
            )
        ...
        # Save figure as SVG
        plt.savefig("posterior_dynamics.svg", format="svg", bbox_inches="tight")
