...
with pm.Model() as model:
    ...
    avg_data = data[['rep1', 'rep2', 'rep3']].mean(axis=1)
    pm.Normal("Y_live", mu=pm.math.log(pm.math.clip(live_solution[:, 1]+dead_solution[:, 1], 1e-8, np.inf)),sigma=sigma_live,
              observed=np.log(avg_data.values))
    avg_dead = death_data[['rep1', 'rep2', 'rep3']].mean(axis=1)
    pm.Normal("Y_dead", mu=pm.math.log(pm.math.clip(dead_solution[:, 1], 1e-8, np.inf)),sigma=sigma_dead,
...
for i in range(n_plot):
    ...
    sol_live = odeint(cells_growth_death, y0, t=time_finer, args=(theta[:-2],), rtol=1e-6, atol=1e-6)
    sol_dead = odeint(cells_growth_death, y0, t=time_finer, args=(theta[:-2],), rtol=1e-6, atol=1e-6)

    plt.subplot(1, 2, 1)
    plt.plot(time_finer, sol_live[:, 0]+sol_live[:,1], '-', color='gray', alpha=0.1)

    plt.subplot(1, 2, 2)
    plt.plot(time_finer, sol_dead[:, 1], '-', color='gray', alpha=0.1)
