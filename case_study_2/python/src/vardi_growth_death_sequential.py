...
def build_pymc_model(ehux_total_time, ehux_total_density,ehux_dead_time, ehux_dead_density ):
    ...
    with pm.Model() as model:
        ...
        # sequential inference prior
        r = pm.Normal(r"$r$ (growth rate)", mu=0.589, sigma=0.036)

        # sequential inference prior
        K = pm.Normal(r"$K$ (carrying capacity)" , mu=3209164, sigma=99822)

        delta = pm.Uniform(r"$\delta$ (death rate)", lower=0.0, upper=0.15)

        # Initial conditions
        T0_init = 236329 #inferred from last example
        D0_init = T0_init*0.12  # roughly from dat
        P0_init = T0_init - D0_init 

        P0 = pm.Normal(r"$P_0$ (init. live)", mu = P0_init , sigma=17918)  # Based on data, around
        D0 = pm.Normal(r"$D_0$ (init. dead)", mu = D0_init , sigma=17918)  # Based on data, around
...
if __name__ == "__main__":
    ...
    file_path = '../res/vardi_logistic_growth_death_chain_seq.nc'
    # Build and run model
    ...
    if plot_dynamics_flag:
        ...
        save_path="../figures/vardi_growth_death_dynamics_seq.png",
        var_properties={
