...
chain = sample(model, NUTS(1000, .95), MCMCSerial(), 1000, 3; progress=true)
...
order = [:mum, :N0, :sigma]
p1 = plot_trace_with_priors(chain; priors=priors, var_order=order, per_chain_density=true)
savefig(p1, "../figures/growth_only_trace.png")  # also per-chain densities
...
savefig(p2, "../figures/growth_only_posterior.png")
