...
def plot_posterior_predictive(ax, trace, time, obs, num_samples=200, ode_solver=solve_cells_ode, **kwargs):
    ...
    for _, row in df.iterrows():
        model_output = ode_solver(row["N0"], time, row["mum"], row["delta"])
...
trace = az.from_netcdf('../data/death_posterior_trace.nc')
...
ax_walks.scatter(mum, delta, s=1, alpha=0.5)
...
az.plot_trace(trace, var_names=["delta"], axes=np.r_[[[ax_hist2],[ax_trace2]]].T, compact=True, show=False)
az.plot_posterior(trace, var_names=["delta"], ax=ax_hist2, show=False)
plot_posterior_predictive(ax_model, trace, time, obs)

plt.tight_layout()

fig.savefig('../figures/figure2')
